...
function TabsList({
  ...
  return (
    <Tabs.List className={twMerge(['flex px-6', className])} {...rest}>
      {children}
    </Tabs.List>
  )
}
  ...
  return (
    <Tabs.Trigger asChild {...rest}>
      {!asChild ? (
        <Button
          className={twMerge([
            '-mr-4 text-nowrap opacity-80 [&>div>svg]:hidden',
            'text-shadow-primary data-[selected]:text-shadow-lg',
...
function TabsContent({
  ...
  return (
    <Tabs.Content
      className={twMerge([
        'data-[selected]:animate-in data-[selected]:fade-in-0 data-[selected]:zoom-in-80 relative min-h-50 w-full px-10 pt-5 pb-10 data-[selected]:duration-500',
